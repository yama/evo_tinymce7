...
* **言語ファイル**: 日本語の UI 言語ファイル（TinyMCE 公式配布物の `ja.js`）を同梱しています。他の言語を追加したい場合は [Tiny Cloud の言語パッケージ](https://www.tiny.cloud/get-tiny/language-packages/) から取得した `langs6/<lang>.js` を `assets/plugins/tinymce7/tinymce/js/tinymce/langs/`（または `assets/plugins/tinymce7/langs/`）へ配置してください。ローカルに存在しない場合は CDN の TinyMCE7 用パッケージ `@tinymce/tinymce-i18n@7`（`langs6/` ディレクトリ）から自動取得します。ローカルファイルを上書きした場合も、更新日時のクエリを付与して読み込むためブラウザーキャッシュを気にせず即時反映されます。
