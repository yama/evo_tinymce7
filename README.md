...
* **言語ファイル**: 日本語などの UI 言語はローカルの `assets/plugins/tinymce7/tinymce/js/tinymce/langs/` に置いた公式 `langs/*.js` を読み込みます（ファイル名は小文字でも読み込めます）。TinyMCE 公式配布の言語パックをダウンロードして配置してください（CDN フォールバックはありません）。
