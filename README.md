...
* **言語ファイル**: 日本語などの UI 言語はローカルの `assets/plugins/tinymce7/tinymce/js/tinymce/langs/`（または `assets/plugins/tinymce7/langs/`）に置いた公式 `langs/*.js` を優先し、存在しない場合は CDN の TinyMCE7 向け言語パック `@tinymce/tinymce-i18n@7`（`langs7/` ディレクトリ）から自動取得します。
