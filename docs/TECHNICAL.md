...
## バージョンポリシー
- 現行のパッケージはベータステータスであり、バージョン番号は `0.x` 系で運用します。
...
3. CDN: `https://cdn.jsdelivr.net/npm/@tinymce/tinymce-i18n@7/langs/<lang>.js`

デフォルトの CDN は TinyMCE7 向けに固定しており、8 系など別メジャーの i18n パッケージを誤って取得しないようにしています。ローカル配置を選ぶ場合は TinyMCE 公式配布物の `langs/` ディレクトリを上記いずれかの場所へそのままコピーしてください（CDN では TinyMCE 7 用に `langs/` を使用しています）。
