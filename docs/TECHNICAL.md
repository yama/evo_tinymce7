...
デフォルトの CDN は TinyMCE7 向けに固定しており、8 系など別メジャーの i18n パッケージを誤って取得しないようにしています。ローカル配置を選ぶ場合は TinyMCE 公式配布物の `langs/` ディレクトリを上記いずれかの場所へそのままコピーしてください（CDN でも TinyMCE 7 用に `langs/` を使用しています）。
